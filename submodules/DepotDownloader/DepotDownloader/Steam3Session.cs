...
namespace DepotDownloader
{
    class Steam3Session
    {
        ...
        private void LogOnCallback(SteamUser.LoggedOnCallback loggedOn)
        {
            ...
            if (isSteamGuard || is2FA || isAccessToken)
            {
                ...
                if (is2FA)
                {
                    do
                    {
                        var code = AuthUiBridge.RequestTwoFactorCode(false);
                ...
                else
                {
                    do
                    {
                        var emailDomain = loggedOn?.EmailDomain;
                        var emailHint = string.IsNullOrWhiteSpace(emailDomain) ? "your email" : $"@{emailDomain}";
                        var code = AuthUiBridge.RequestEmailCode(emailHint, false);
                        if (string.IsNullOrWhiteSpace(code))
                        {
        ...
        {
            AuthUiBridge.NotifyQrCode(challengeUrl);
